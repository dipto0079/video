      ...
      <div class="panel panel-border panel-primary">
          ...
          <div class="panel-body">                 
            <table class="table table-bordered" id="video-list">
              <?php $video_files = $this->db->get_where('video_file', array('videos_id'=> $param1))->result_array();
                    foreach($video_files as $video_file):
               ?>
               <tr id="row_<?php echo $video_file['video_file_id']; ?>"><td><strong><?php echo $video_file['file_source']; ?></strong></td><td><?php echo urldecode($video_file['file_url']); ?></a></td><td><a title="delete" class="btn btn-icon" onclick="delete_row('video_file',<?php echo urldecode($video_file['video_file_id']);?>)" class="delete"><i class="fa fa-remove"></i></td></tr>             
            <?php endforeach; ?>
            </table>
            <?php echo form_close(); ?>          
        </div>
            ...
            if (window.File && window.FileReader && window.FileList && window.Blob) {
                ...
                switch (ftype) {
                    ...
                    default:
                        ...
                        //$("#output").html("<b>" + ftype + "</b> fsdfUnsupported file type!");
                        return false
                }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (post_status == "success") {
                      var html_text ='<tr id="row_'+row_id+'"><td><strong>'+type+'</strong></td><td>'+url+'</a></td><td><a title="delete" class="btn btn-icon" onclick="delete_row('+"'video_file',"+row_id+')" class="delete"><i class="fa fa-remove"></i></td></tr>';
