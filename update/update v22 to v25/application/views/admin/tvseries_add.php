        ...
        <div class="col-lg-6 col-md-offset-3">
            <div class="panel panel-border panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title">Import TV-Series</h3>
                </div>
...
<script type="text/javascript">
    function loadAllActor() {
        ...
        $.ajax({
            ...
            success:function(response) {
                ...
                //     //$("#actor").append("<option value='"+id+"'>"+name+"</option>").trigger('change');
                }
    ...
    function load_actor() {
        $.get('<?php echo base_url()."admin/load_stars2"; ?>', function(data) {
            ...
            $("#actor").empty('');
            $('#actor').val('');            
            //$("#actor").select2("val", "").trigger("change");
            $("#actor").html(data);
            console.log(data);
            $("#actor").select2("destroy");
    ...
    function load_star(){
        ...
        $.ajax({
            ...
            success:function(response) {
                ...
                // return response;
                //$('#actor').val(null).trigger('change');
                //JSON.parse(response);
                var len = response.length;
                //alert(len);
                // //alert(response);
                // // $("#actor").select2('destroy').empty().select2({data: [{id: 1, text: 'new text'}]});
                // // initSelect();
                // //$('#actor').select2('destroy');
                $("#actor").empty();
                for( var i = 0; i<len; i++){
