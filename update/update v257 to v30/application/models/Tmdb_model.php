...
class Tmdb_model extends CI_Model {
  ...
  function update_actors($actors){
    ...
    for ($i=0; $i<sizeof($actors); $i++) {      
      $actors_name        = preg_replace('/[^A-Za-z0-9\-]/', '', trim($actors[$i]['name']));
  ...
  function update_directors_writers($stars){
    ...
    for ($i=0; $i<sizeof($stars); $i++) {      
      $actors_name        = preg_replace('/[^A-Za-z0-9\-]/', '', trim($stars[$i]['name']));
  ...
  function search($title='',$to=''){
      ...
      if($to =='tv'):
        ...
        $data           = file_get_contents('http://ovoo.spagreen.net/scrapper/v30/search_movie/'.$purchase_code.'/'.$title);
      endif;
      $data           = json_decode($data, true);
